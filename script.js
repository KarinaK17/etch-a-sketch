...
function startAgain() {
    ...
    pixelsPerSide=prompt("Please enter number of pixels per side for the new game!", 64);
    pixelsPerSide=Number(pixelsPerSide);
    container.style.cssText=`grid-template-columns: repeat(${pixelsPerSide}, 1fr)`;
    game(pixelsPerSide);

    if (pixelsPerSide<0||pixelsPerSide==null||pixelsPerSide>100||!Number.isFinite(pixelsPerSide)) {
